...
In order to create or delete partitions a full description of the system must be obtained by a call to [IoReadPartitionTableEx](/windows-hardware/drivers/ddi/ntddk/nf-ntddk-ioreadpartitiontableex). The drive layout structure can be modified by the system format utility to reflect a new configuration of the disk.
